...
export async function createUserHandler(
  ...
  if (user) {
    return reply.status(401).send({ message: 'user with this email already exists', error: true })
  }
  ...
  try {
    ...
    return reply.status(500).send({ message: "user creation failed", error: true })
  }
